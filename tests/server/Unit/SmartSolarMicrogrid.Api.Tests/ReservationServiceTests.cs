 ...
 * Description: Unit tests for ReservationService business rules (7-day rule, 12-hour rule).
 */
...
namespace SmartSolarMicrogrid.Api.Tests
{
    public class ReservationServiceTests
    {
        ...
        public async Task CancelReservation_Succeeds_WhenMoreThan12HoursRemaining()
        {
            ...
            _mockRepo.Setup(r => r.GetByIdAsync(reservationId))
                     .ReturnsAsync(existingReservation);
            _mockRepo.Setup(r => r.UpdateAsync(reservationId, It.IsAny<EnergyReservation>()))
                     .Returns(Task.CompletedTask);
            ...
            // Assert
            _mockRepo.Verify(r => r.UpdateAsync(reservationId, It.Is<EnergyReservation>(e => e.Status == "Cancelled" && e.CancelReason == "Change of plans")), Times.Once);
        }
    }
}
